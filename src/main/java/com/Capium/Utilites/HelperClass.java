...
//	}
	...
	public static void hoverAndClickActionOnRow(int rowIndex, String actionType) {
		...
		WebElement table = wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//table[@class='e-table']")));

		List<WebElement> rows = table.findElements(By.tagName("tr"));

		if (rows.isEmpty()) {
			throw new IllegalStateException("Quickk entry Table has no rows. Cannot perform action.");
		}
		...
		while (!success && retries < 3) {
			try {
				...
				case "view":
					buttonLocator = By.xpath("//button[@title='View']");
					break;
				case "export":
					buttonLocator = By.xpath("//button[@title='Export']");
					break;
				case "edit":
				...
				WebElement actionButton = wait.until(ExpectedConditions.elementToBeClickable(buttons.get(1)));
				actionButton.click();
...
// Mouse hover 
	...
	// General Methods like Sendkeys, Click , List
		...
		public static void selectItemsPerPage(WebDriver driver, WebDriverWait wait, String valueToSelect) {
		    // 1. Click the dropdown to open the options
		    By dropdownInput = By.xpath("//input[@aria-label='dropdownlist']");
		    WebElement dropdown = wait.until(ExpectedConditions.elementToBeClickable(dropdownInput));
		    dropdown.click();
           sleep1(TIMEOUT);
		    // 2. Wait for the dropdown options to be visible
		    // The dropdown options are rendered in a separate popup (floating div)
		    String dropdownOptionXpath = String.format("//ul[contains(@class, 'e-ul')]/li[text()='%s']", valueToSelect);
		    WebElement option = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(dropdownOptionXpath)));

		    // 3. Click the desired option
		    option.click();
		}

		public static void selectPeriodFromDropdown(WebElement dropdownElement, String periodToSelect, Integer index) {
			try {
				waitForPageToLoad(driver);
				safeClick(dropdownElement, "Click");
				WebDriverWait wait = new WebDriverWait(HelperClass.getDriver(), Duration.ofSeconds(5));
				List<WebElement> options = wait.until(
						ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//ng-dropdown-panel//div[@role='option']/span[normalize-space()='"+periodToSelect+"']")));
				if (options.isEmpty()) {
					System.out.println("No options found in the Period dropdown.");
					return;
				}
				boolean selected = false;
				if (periodToSelect != null && !periodToSelect.trim().isEmpty()) {
					for (WebElement option : options) {
						String optionText = option.getText().trim();
						if (optionText.equalsIgnoreCase(periodToSelect.trim())) {
							option.click();
							System.out.println("Selected specific period: " + optionText);
							selected = true;
							break;
						}
					}
				}
				if (!selected && index != null && index >= 0 && index < options.size()) {
					options.get(index).click();
					System.out.println("Selected period by index " + index + ": " + options.get(index).getText());
					selected = true;
				}
				if (!selected) {
					options.get(0).click();
					System.out.println("Selected first period from dropdown: " + options.get(0).getText());
				}
			} catch (Exception e) {
				System.out.println("Error selecting period from dropdown: " + e.getMessage());
			}
		}
        ...
        for (int value : dropdownValues) {
            try {
                ...
                WebElement dropdown = wait.until(ExpectedConditions.elementToBeClickable(
                    By.xpath("//span[@role='combobox' and @aria-label='dropdownlist']")
                ));
                dropdown.click();
                ...
                // Wait for table to update (better to wait dynamically)
                Thread.sleep(1500);

                // Count the rows
                List<WebElement> rows = driver.findElements(
                    By.xpath("//table[contains(@class,'e-table')]/tbody/tr")
                );
